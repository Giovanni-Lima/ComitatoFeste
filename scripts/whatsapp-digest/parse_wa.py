...
# Il nome file può contenere spazi (documenti allegati con un nome scelto
# dall'utente, non auto-generato da WhatsApp come IMG-/PTT-/VID-/STK-) — bug
# trovato il 14/9/2026 con "elenco aziende per contributo feste.pdf": senza
# spazio nella classe di caratteri il file veniva letto come testo normale,
# non riconosciuto come media.
FILE_RE = re.compile(r"^([A-Za-z0-9_\-][A-Za-z0-9_\- ]*\.[A-Za-z0-9]+) \(file allegato\)$")
...
def parse_messages(txt_path):
    """Parsa il .txt esportato da WhatsApp in una lista di messaggi
    {date, time, sender, kind, text, file, edited}. Riusata sia dallo
    script standalone sotto sia da transcribe_new.py (recupero automatico
    dei messaggi nuovi + trascrizione, vedi CLAUDE.md)."""
    messages = []
    cur = None
